...
const VideoDetails = () => {
  ...
  const fetchRelatedVideosData = () => {
    ...
    fetchDataFromApi(`video/related-contents/?id=${id}`)
      ...
      .catch(() => {
        console.log("cannot fetch related contents");
      });
  };

  return (
    <div className="flex  justify-center items-baseline flex-row h-[calc(100%-56px)]  bg-black overflow-y-auto">
          ...
          <div className="flex justify-between flex-col md:flex-row mt-4">
            <div className="flex">
              ...
              <div className="flex flex-col ml-3">
                ...
                <div className="text-white/[0.7] text-sm">
                  {video?.data?.author?.stats?.subscribersText}
                </div>
              </div>
            </div>
            <div className="flex text-white mt-4 md:mt-0">
              <div className="flex items-center justify-center h-11 px-6 rounded-3xl bg-white/[0.15]">
                ...
                {`${abbreviateNumber(video?.data?.stats?.views, 2)} Likes`}
              </div>
